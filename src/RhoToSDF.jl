...
struct Rho2sdfOptions
  ...
  function Rho2sdfOptions(;
    ...
    new(
      ...
      sdf_grid_setup,
      export_input_data,
      export_nodal_densities,
      export_raw_sdf,
      rbf_interp,
      rbf_grid,
      remove_artifacts,
      artifact_min_component_ratio,
      export_analysis,
      element_type
    )
  end
...
function rho2sdf(
  ...
  if options.remove_artifacts
    ...
    if options.export_analysis
      ...
      exportSdfToVTI(
        taskName * "_SDF_raw_$(element_name)_B-$(B).vti",
        sdf_grid,
        sdf_dists,
        "distance"
      )
    ...
    if nodes_flipped > 0 && options.export_analysis
      B = round(sdf_grid.cell_size, digits = 4)
      exportSdfToVTI(
        taskName * "_SDF_cleaned_$(element_name)_B-$(B).vti",
        sdf_grid,
        sdf_dists,
        "distance"
      )
  ...
  if options.export_raw_sdf
    B = round(sdf_grid.cell_size, digits = 4)
    exportSdfToVTI(
      taskName * "_SDF_$(element_name)_CellSize-" * string(B) * ".vti",
      sdf_grid,
      sdf_dists,
      "distance"
    )
  ...
  (fine_sdf, fine_grid) =
    RBFs_smoothing(mesh, sdf_dists, sdf_grid, options.rbf_interp, smooth, taskName)

  elapsed_time = time() - start_time
  print_data("Post-processing time: $(round(elapsed_time, digits=2)) seconds")

  # Export final smoothed results with element type in filename
  export_sdf_results_with_element_type(
    fine_sdf,
    fine_grid,
    sdf_grid,
    taskName,
...
"""
function export_sdf_results_with_element_type(
  fine_sdf,
  fine_grid,
  my_grid,
  taskName,
  ...
  exportSdfToVTI(
    "$(taskName)_$(element_name)_B-$(B)_smooth-$(smooth)_$(name).vti",
    my_grid,
    fine_LSF_offset,
    "distance",
    smooth
