...
# ----------------------------
function locate_point_in_mesh(mesh::BlockMesh, p::SVector{3, Float64})
  # Získání hranic mřížky
  ...
  # Výpočet indexů buňky (floating point)
  i_f = r[1] * (mesh.nx - 1) + 1
  j_f = r[2] * (mesh.ny - 1) + 1
  k_f = r[3] * (mesh.nz - 1) + 1
  
  # Základní indexy buňky obsahující bod p
  i0 = clamp(floor(Int, i_f), 1, mesh.nx-1)
  j0 = clamp(floor(Int, j_f), 1, mesh.ny-1)
  k0 = clamp(floor(Int, k_f), 1, mesh.nz-1)
  
  # Lokální souřadnice v rozsahu [-1,1]
  ξ₁ = 2.0 * (i_f - i0) - 1.0
  ξ₂ = 2.0 * (j_f - j0) - 1.0
  ξ₃ = 2.0 * (k_f - k0) - 1.0
  
  # Výpočet velikostí buňky (potřebné pro gradient)
  Δh = (vmax .- vmin) ./ (SVector(mesh.nx, mesh.ny, mesh.nz) .- 1)
  
  return (
      indices = (i0, j0, k0),      # Základní indexy buňky
      local_coords = [ξ₁, ξ₂, ξ₃], # Lokální souřadnice
      cell_sizes = Δh              # Velikosti buňky
  )
end

function eval_sdf(mesh::BlockMesh, p::SVector{3, Float64})
  # Lokalizace bodu v mřížce
  loc = locate_point_in_mesh(mesh, p)
  
  # Výpočet tvarových funkcí
  N = hex8_shape(loc.local_coords)
  
  # Získání a interpolace SDF hodnot
  sdf_values = get_cell_sdf_values(mesh, loc.indices...)
  return sum(N .* sdf_values)
end

function eval_sdf_gradient(mesh::BlockMesh, p::SVector{3, Float64})
  # Lokalizace bodu v mřížce
  loc = locate_point_in_mesh(mesh, p)
  ...
  compute_hex8_shape!(N, d¹N_dξ¹, loc.local_coords...)
  ...
  for i in 1:8
      gradient = gradient .+ sdf_values[i] .* SVector{3,Float64}(
          d¹N_dξ¹[i,1] * (2.0/loc.cell_sizes[1]),
          d¹N_dξ¹[i,2] * (2.0/loc.cell_sizes[2]),
          d¹N_dξ¹[i,3] * (2.0/loc.cell_sizes[3])
      )
  end
...
function warp_node_to_isocontour!(mesh::BlockMesh, node_index::Int, max_iter::Int)
  ...
  for iter in 1:max_iter
      ...
      grad = eval_sdf_gradient(mesh, current_position)
