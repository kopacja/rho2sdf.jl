...
# - Handle all three extensions (.vtu, .vti, .vtk, .jld2, .png, .vtr) in a single operation
...
delete_files() {
  ...
  files=$(find "$dir" -maxdepth 1 -type f \( -name "*.vtu" -o -name "*.vti" -o -name "*.vtk" -o -name "*.jld2" -o -name "*.png" -o -name "*.vtr" \))
  ...
  if [ -n "$files" ]; then
    ...
    if [ "$answer" = "y" ]; then
      ...
      find "$dir" -maxdepth 1 -type f \( -name "*.vtu" -o -name "*.vti" -o -name "*.vtk" -o -name "*.jld2" -o -name "*.png" -o -name "*.vtr" \) -delete
