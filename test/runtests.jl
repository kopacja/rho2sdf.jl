...
@testset "Rho2sdf.jl" begin
  ...
  if (RUN_CUBE_MESHES)
    @time @testset "SphereInCube-Meshes" begin include("PrimitiveGeometriesTest/SphereInCube-Meshes.jl") end

  end

  if (RUN_COMP_TESTS)
    @time @testset "HexBlockSdfTest" begin include("HexBlockSdfTest.jl") end
    @time @testset "HexSphereSdfTest" begin include("HexSphereSdfTest.jl") end
  end

  if (RUN_CONV_TESTS)
    @time @testset "SphereConvergenceTest" begin include("ConvergenceTests/SphereConvergenceTest.jl") end
    @time @testset "CubeConvergenceTest" begin include("ConvergenceTests/CubeConvergenceTest.jl") end
  end
...
if (RUN_1HEX_EL)
  @testset "1hex_el" begin
    ...
    ρₜ = 0.5  # Threshold density (isosurface level)
      rho = [0., 0.] # Elementy density (something for Mesh struct)
